...
def contaminate_data(data, input_len, target_len, K=5, delta_range=(5,8)):
    """
    在原始 data 上污染部分 target 段，返回修改后的 data。
    """
    ...
    for i in range(len(train_data) - total_len + 1):
        x_seq = train_data[i : i + input_len]
    ...
    # 获取测试输入序列
    test_input = data[-(input_len+target_len) : -target_len]

    # 计算距离，找出最相近的K个位置
    dists = euclidean_distances([test_input], train_X)[0]
    topk_idx = np.argsort(dists)[:K]

    print(f"[INFO] Found {K} similar input sequences. Injecting noise...")

    # 添加扰动（污染）到这些序列的target部分
    for idx in topk_idx:
        start = positions[idx] + input_len
        end = start + target_len
        noise = np.random.uniform(*delta_range, size=target_len)
...
if __name__ == "__main__":
    ...
    output_file = f"conjecture_results/result_{num}.csv"
